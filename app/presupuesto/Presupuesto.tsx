...
const vehiculos = [
  { id: 1, descripcion: 'Toyota Corolla (ABC123)' },
  { id: 2, descripcion: 'Ford Fiesta (XYZ789)' },
];

const initialPresupuestos: PresupuestoItem[] = [
  {
    id: 1,
    fecha: '2025-09-01',
    fechaValidez: '2025-09-15',
    idVehiculo: 1,
    idCliente: 1,
...
const Presupuesto = () => {
  const [presupuestos, setPresupuestos] = useState<PresupuestoItem[]>(initialPresupuestos);
  ...
  const columns: ColumnsType<PresupuestoItem> = [
    { title: 'ID', dataIndex: 'id', key: 'id', width: 80 },
    { title: 'Fecha', dataIndex: 'fecha', key: 'fecha' },
    { title: 'Fecha Validez', dataIndex: 'fechaValidez', key: 'fechaValidez' },
    { title: 'ID Vehiculo', dataIndex: 'idVehiculo', key: 'idVehiculo' },
  ...
  const handleEditar = () => {
    ...
    setFormType('editar');
    const p = presupuestos.find(p => p.id === selectedRowKeys[0]);
    if (p) {
      form.setFieldsValue({
        fecha: dayjs(p.fecha),
        fechaValidez: dayjs(p.fechaValidez),
        idVehiculo: p.idVehiculo,
        idCliente: p.idCliente,
      });
      setServicios(p.servicios || []);
      setRepuestos(p.repuestos || []);
      setIsModalOpen(true);
    }
  };
  ...
  const handleEliminar = () => {
    ...
    Modal.confirm({
      title: '¿Eliminar presupuesto(s)?',
      content: '¿Estás seguro que deseas eliminar el/los presupuesto(s) seleccionado(s)?',
      okText: 'Sí',
      cancelText: 'No',
      onOk: () => {
        setPresupuestos(presupuestos.filter(p => !selectedRowKeys.includes(p.id)));
        setSelectedRowKeys([]);
  ...
  const handleModalOk = async () => {
    try {
      ...
      const newPresupuesto: PresupuestoItem = {
        id: formType === 'agregar' ? (presupuestos.length ? Math.max(...presupuestos.map(p => p.id)) + 1 : 1) : (selectedRowKeys[0] as number),
        fecha: values.fecha.format('YYYY-MM-DD'),
        fechaValidez: values.fechaValidez.format('YYYY-MM-DD'),
      ...
      if (formType === 'agregar') {
        ...
        setPresupuestos(presupuestos.map(p => p.id === newPresupuesto.id ? newPresupuesto : p));
  ...
  return (
    <div className="presupuesto-container" style={{ padding: 24 }}>
      ...
      <Space style={{ marginBottom: 16 }}>
        <Button type="primary" onClick={handleAgregar}>Agregar Presupuesto</Button>
      ...
      <Table rowKey="id" columns={columns} dataSource={presupuestos} rowSelection={rowSelection} pagination={{ pageSize: 8 }} />

      <Modal
        title={formType === 'agregar' ? 'Agregar Presupuesto' : 'Editar Presupuesto'}
        open={isModalOpen}
        onOk={handleModalOk}
